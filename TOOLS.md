...
- **新规则（2026-03-26 晚补充）**：用户所有交易 / 行情 / 图表类图片，默认必须在 **Opus 已确认执行** 的前提下分析
- 若当前 live chat session 中：
  - 未出现用户前台可见的 Opus 切模成功回执
  - 或当前模型核对结果不是 Opus
  则助手**不得继续输出正式交易分析正文**
- 在上述情况下，助手必须改为明确提示用户按以下顺序操作：
  1. 先缓存当前上下文
  2. 新建聊天
  3. 切换到 `anthropic/claude-opus-4-6`
  4. 重新发图后再开始正式分析
- 在上述情况下，**不得**再给“模型未确认下的临时交易分析”
- 非交易类普通图片理解，不受这条限制；这条限制仅适用于交易 / 行情 / 图表分析
